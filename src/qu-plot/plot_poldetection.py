...
for i in range(int(np.max(bnum) + 1)):
    ...
    ax3.errorbar(
        pos, val, yerr=std, xerr=[[xlim1], [xlim2]], fmt=".", capsize=3, c="lightgray"
    )
...
for i in range(0, len(phi)):
    fspec.append(
        calc_f(phi[i], data.l2[::-1], data.stokesQn[::-1], data.stokesUn[::-1], w)
    )
...
ax5.text(
    320,
    0.95 * np.max(np.abs(fspec)),
    r"peak @ {:.2f} rad/m$^2$".format(phi0),
    color="gray",
    fontsize=12,
)
...
pfit = fit_chi(data.l2[::-1], p_angl[::-1], data.noise, [phi0, 0.0])
...
ax4.text(
    l2mid - 0.015,
    1.4,
    r"d$\chi$/d$\lambda^2$ = {:.2f} rad/m$^2$".format(0.5 * pfit[0]),
    color="gray",
    fontsize=12,
)
